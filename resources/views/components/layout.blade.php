...
<body class="relative flex flex-col min-h-screen overflow-auto bg-bgcolor-800 font-appfont scrollbar-thin scrollbar-thumb-scrollthumb">
    ...
    @if (session()->has('success'))
        <div class="fixed px-4 py-2 text-base text-black bg-bgcolor-900 rounded-xl animate-goesInAndOut" id="flash-message">
            <p>{{ session('success') }}</p>
        </div>
    @endif

    @if (session()->has('failure'))
        <div class="fixed px-4 py-2 text-base text-red-500 bg-bgcolor-900 rounded-xl animate-goesInAndOut" id="flash-message">
            <p>{{ session('failure') }}</p>
        </div>
    @endif
