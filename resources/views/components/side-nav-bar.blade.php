<nav class="sticky top-0 flex-col hidden w-full col-span-3 col-start-1 gap-1 p-1 text-lg max-h-[100vh] md:flex">
    ...
    <main class="w-full h-full overflow-auto scrollbar-thin">
        <h3 class="w-full py-1 text-xl font-bold text-gray-900">Categories:</h3>
        @foreach ($categories as $category)
            <a href="?category={{ $category->slug }}" class="block my-1 w-full px-4 py-1 text-gray-900 rounded-md hover:bg-bgcolor-900 {{ request('category') == $category->slug ? 'bg-bgcolor-900' : '' }} first-letter:uppercase">{{ $category->name }}</a>
        @endforeach
    </main>
</nav>
