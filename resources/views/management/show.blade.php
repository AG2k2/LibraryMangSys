<x-layout>
    ...
    <section class="flex flex-col items-start justify-center w-full p-2 pt-8 mb-auto md:p-4 lg:p-6">
        ...
        <header class="z-50 flex items-end justify-center gap-3 mb-5 reltive top-28">
            <div>
                <img src="{{ asset('storage/' . $employee->pro_pic) }}" alt="#" class="relative border-4 rounded-full border-bgcolor-850 w-52 h-52 bg-bgcolor-850">
            </div>
            <div class="flex flex-col items-start h-full ">
                <h3 class="px-3 my-2 text-xl text-gray-900 ">
                    <span class="font-bold">ID:</span> {{ $employee->id  }}
                </h3>
                <h3 class="px-3 my-2 text-xl text-gray-900 ">
                    <span class="font-bold">Full name:</span> {{ ucwords($employee->first_name . ' ' . $employee->last_name) }}
                </h3>
                <h3 class="px-3 my-2 text-xl text-gray-900 ">
                    <span class="font-bold">Username:</span> {{ '@'. $employee->username  }}
                </h3>
            </div>
        </header>
            ...
            <div class="w-full p-3 border bg-bgcolor-700 rounded-xl border-bgcolor-850">
                <table class="text-lg">
                    <tr>
                        ...
                        <td class="w-full p-2 pl-4 rounded-t-xl bg-bgcolor-800">{{ $employee->gender == 'm' ? 'Male' : 'Female' }}</td>
                    </tr>
                    <tr>
                        ...
                        <td class="w-full p-2 pl-4 bg-bgcolor-800">
                            ...
                            {{ $employee->email_verified_at == null ? 'bg-red-400' : 'bg-green-400'  }}">
                            {{ $employee->email_verified_at == null ? 'unverified' : 'verified' }}
                            </span>
                        </td>
                    </tr>
                    <tr>
                        ...
                        <td class="w-full p-2 pl-4 bg-bgcolor-800">
                        @if ($employee->enroll_submitted_at != null)
                                <span class="p-1 text-lg text-black rounded-xl">
                                    Submitted at {{ $employee->enroll_submitted_at }}
                                </span>
                            @else
                                <span class="p-1 text-lg text-black rounded-xl">
                                    Not submitted
                    ...
                    <tr>
                        ...
                        <td class="w-full p-2 pl-4 bg-bgcolor-800">
                            {{ Carbon\Carbon::parse($employee->birth_date)->format('Y / m / d') }} - ({{ Carbon\Carbon::parse($employee->birth_date)->age }} years old)
                        </td>
                    </tr>
                    <tr>
                        <td class="p-2 md:whitespace-nowrap">Joined:</td>
                        <td class="w-full p-2 pl-4 bg-bgcolor-800 rounded-b-xl">
                            {{ $employee->created_at->format('Y / m / d') }} - ({{ $employee->created_at->diffForHumans() }})
                        </td>
                    </tr>
                </table>
            </div>
                    ...
                    @foreach ($books as $book)
                        ...
                        <li class="grid w-full grid-cols-12 p-1 my-1 text-base text-center bg-gray-200 rounded-md hover:bg-gray-300">
                            ...
                            <div class="col-span-2 truncate" >
                                @if (! $employee->books->isEmpty() )
                                    @if ($employee->books->first()->pivot->return_at !== null)
                                        <p class="text-sm text-black ">
                                            <span>
                                                {{ Carbon\Carbon::parse($employee->books->first()->pivot->return_at)->format('Y / m / d') }}
                                            </span>
                                        </p>
                                    @elseif($employee->books->first()->pivot->taken_at !== null)
                                        <p class="text-sm text-white rounded-md">
                                            <?php
                                                $borrowDate = Carbon\Carbon::parse($employee->books->first()->pivot->taken_at);
